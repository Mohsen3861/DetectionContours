...
int ImageSaveSCI(char *filename, Image *image){
  FILE *file;
  struct {
    short   MAGIC;
    char    STORAGE;
  ...
  header.BPC=1;
  header.DIMENSION=2;
  header.XSIZE=image->sizeX;
  header.YSIZE=image->sizeY;
  header.ZSIZE=0;
  header.PIXMIN=0;
  header.PIXMAX=255;
  snprintf(header.DUMMY,1,"0");
  snprintf(header.IMAGENAME,sizeof(filename),"%s",filename);
  header.COLORMAP=0;
  snprintf(header.DUMMY,1,"0");

  fwrite(&header,512,1,file);
  fwrite(&image->data,sizeof(image->data),1,file);
  fclose(file);
  return 1;
}


int ImageLoadSCI(char *filename, Image *image){
  FILE *file;
  struct {
    short   MAGIC;
    char    STORAGE;
  ...
  image->sizeX=header.XSIZE;
  image->sizeY=header.YSIZE;
  image->data=(GLubyte *) malloc(image->sizeX*image->sizeY*3);
  fread(&image->data,sizeof(image->data),1,file);
  fclose(file);
  return 1;
}
